...
const QuickActions = () => {
  const quickActions = [{
    ...
    label: 'Personal',
    path: '/personal',
    tooltip: 'Personal banking solutions',
    showOnMobile: false
  }, {
    icon: Building2,
    label: 'Business',
    path: '/business',
    tooltip: 'Business banking services',
    showOnMobile: false
  }, {
    icon: TrendingUp,
    label: 'Investments',
    path: '/investments',
    tooltip: 'Investment opportunities',
    showOnMobile: false
  }, {
    icon: GraduationCap,
    label: 'Education',
    path: '/education',
    tooltip: 'Educational banking services',
    showOnMobile: false
  }, {
    icon: BookOpen,
    label: 'Learn',
    path: '/learn',
    tooltip: 'Financial education resources',
    showOnMobile: false
  }];
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative">
        ...
        <div className="lg:hidden flex items-center justify-center space-x-1 py-3">
          {mobileActionButtons.map(action => <Link key={action.label} to={action.path}>
              <Button variant="ghost" size="sm" className="text-white hover:text-[#ffd700] hover:bg-gray-800 flex items-center space-x-1 px-2 py-2 h-10 mx-[20px]">
        ...
        <div className="hidden lg:flex items-center justify-center space-x-8 py-3">
          {quickActions.map(action => <Link key={action.label} to={action.path} className="group flex items-center space-x-2 px-4 py-2 rounded-full bg-transparent hover:bg-gray-800 transition-all duration-200 relative" title={action.tooltip}>
